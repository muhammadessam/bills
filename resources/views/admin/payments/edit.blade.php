...
@section('title', 'المدير / المدفوعات / تحرير')
@section('content')
            ...
            <div class="widget-header">
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <h4>تعديل مدفوعة جديدة</h4>
                    </div>
                </div>
            </div>
            <div class="widget-content widget-content-area">
                <form action="{{route('admin.payment.update', $payment)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="amount">القمية :</label>
                            <input type="number" class="form-control" id="amount" name="amount" value="{{old('amount', $payment->amount)}}" placeholder="قيمة المدفوعات ">
                            @error('amount')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" class="form-control" id="released_at" name="released_at" value="{{old('released_at',$payment->released_at->format('Y-m-d'))}}" placeholder="تاريخ الانشاء">
                            @error('released_at')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col">
                            ...
                            <select class="form-control selectpicker" data-live-search="true" name="bill_id" id="bill_id">
                                @foreach(\App\Models\Bill::all() as $bill)
                                    <option value="{{$bill['id']}}" {{$bill['id'] === $payment['bill_id'] ? 'selected' : ''}}>{{$bill['number']}}</option>
                                @endforeach
                            </select>
                            @error('bill_id')
                            <span class="text-danger">{{$message}}</span>
...
@push('js')
    <script>
        ...
        @if ($payment->hasMedia('payments'))
        firstUpload.addImagesFromPath(["{{$payment->getFirstMediaUrl('payments')}}"]);
        @endif
    </script>
@endpush
